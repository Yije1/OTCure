...
class Medication:
    ...
    def __init__(self, name, description, usage, ingredients, class_type, effect_group, preg, caffe, age, url):
        ...
        self.class_type = class_type  # 예: "진통제", "감기약", "소화제"
        self.effect_group = effect_group # 예: "Acetaminophen", "Ibuprofen", "Antihistamine"
        self.preg = preg # 0: 해당없음, 1: 임부 주의, 2: 임부 금기
        self.caffe = caffe
        self.age = age # 0: 해당없음, 1: 고령자 주의
        self.url = url
...
WARNING_RULES = {
    ...
    "Tylenol_Cold_Combination": {
        "type": "drug_name_set",
        "names": ["타이레놀500mg", "타이레놀콜드에스정"],
        "message": "🚨 **유사한 약을 중복**으로 섭취하게 됩니다. 두 약 모두 해열/진통 효과가 있어 권장되지 않습니다.",
        "level": "error"
    },
    "NSAID_Aspirin_Conflict": {
        "type": "effect_group_set",
        "groups": ["Ibuprofen", "Aspirin"], 
        "message": "⚠️ **NSAIDs(이부프로펜 계열)와 아스피린을 함께 복용**하면 위장 출혈 위험이 높아집니다.",
        "level": "warning"
    },
    "Multiple_Antihistamine": {
...
def on_log_save(selected_names, log_time_key, log_desc_key):
    ...
    선택된 약물을 기록하고 일일 최대 복용량을 검사하며, 성공 시 체크박스를 초기화합니다.
    """
...
st.sidebar.info(
    f"**{profile['name']}**님 프로필:\n"
    f"나이: {profile['age']}세 ({profile['ageornot']}), 성별: {profile['gender']}\n"
    f"임신여부: {profile['pregnant']}"
)
...
with tab_ingredient_info:
    ...
    st.write("데이터베이스에 등록된 모든 성분과 해당 성분을 포함하는 약물 목록입니다.")
    ...
    for ing in SORTED_INGREDIENTS:
        with st.expander(f"**{ing}**"):
            ...
            if meds_with_ing:
                ...
                st.caption("포함된 약물 정보가 없습니다.")

# --- [Feature 3] "성분으로 약물 제외" 탭 ---
with tab_ingredient_exclude:
    st.subheader("🚫 특정 성분 포함 약물 제외하기")
    st.info("여기서 성분을 선택하면 '약물 선택 및 기록' 탭에서 해당 성분이 포함된 약물의 체크박스가 자동으로 비활성화됩니다.")
    ...
    st.markdown("---")
    st.write("성분 선택 후 '약물 선택 및 기록' 탭으로 돌아가 체크박스가 비활성화된 것을 확인하세요.")
...
with tab_selection:
    ...
    profile = st.session_state['user_profile']
    is_pregnant = profile['pregnant'] in ["임신 중", "수유 중"]
    ...
    # 9. 복용 기록 저장 폼
        st.markdown("---")
        st.subheader("📝 복용 기록 저장하기")
        st.write(f"선택된 약물 **({len(selected_med_names)}개)**의 복용 시간과 간단한 설명을 기록합니다.")
        ...
        with st.form(key='log_form', clear_on_submit=True):
            ...
            st.form_submit_button(
                label=f"✅ 선택된 {len(selected_med_names)}개 약물 복용 기록 저장",
